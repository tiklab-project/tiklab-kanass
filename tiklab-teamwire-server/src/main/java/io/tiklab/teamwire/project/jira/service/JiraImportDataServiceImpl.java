...
import io.tiklab.teamwire.project.jira.util.UncompressUtil;
...
public class JiraImportDataServiceImpl implements JiraImportDataService {
    ...
    @Override
    @Transactional
    public String importJireData(InputStream inputStream) {
...
//            SAXReader saxReader = new SAXReader();
            ...
            parser.parse(new File(path), saxParseService);
            
            //InputStream resourceAsStream = Thread.currentThread().getContextClassLoader().getResourceAsStream("entities.xml");
            List<org.w3c.dom.Element> elements = saxParseService.getElementList();
    ...
    @Override
    public String saxParse(InputStream inputStream) {
        BufferedReader unZIP=null;
        try {
            unZIP = new UncompressUtil().unZIP(inputStream, unzipAddress );
            String path=unzipAddress+"/entities.xml";
            ...
            SAXParser parser = factory.newSAXParser();
            parser.parse(new File(path), saxParseService);
            for (org.w3c.dom.Element attributes : saxParseService.getElementList()) {
                String name = attributes.getAttribute("name");
                System.out.println("jarasss" + name);
            }
    ...
    public void setProject(Element element, String createUserId){
        ...
        if (ObjectUtils.isEmpty(project) && projecttype.equals("software")){
            ...
            project1.setProjectKey("jr" + originalkey);
    ...
    public Map<String, String> setProjectRole(String projectId){
        ...
        try {
            for (Role role : roleList) {
                ...
                String newRoleId = roleService.cloneRole(role.getId(), "teamwire");
